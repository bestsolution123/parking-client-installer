        ...
        <div class="col-xl-6">
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    @endif
                    <form method="POST" action="/dashboard/manlessPayment/edit/{{ encrypt($manlessPayment->id) }} "
                        enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3">
                            <label class="form-label" for="basic-default-fullname">Nama Pembayaran</label>
                            <input type="text" class="form-control" id="basic-default-fullname"
                                placeholder="Nama Pembayaran" name="name" value="{{ $manlessPayment->name }}" />
                        </div>
                        <div class="mb-3">
                            ...
                            ?>
                                <option value="{{ $content[$i] }}"
                                    {{ $manlessPayment->payment_type == $content[$i] ? 'selected' : '' }}>
                                    {{ $data[$i] }}
                                </option>
                                <?php
                        ...
                        <div class="mb-3">
                            ...
                            <input type="hidden" class="form-control" id="payment_bank_old" name="payment_bank_old"
                                value="{{ $manlessPayment->payment_bank }}" />
    ...
    <script>
        $(document).ready(function() {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                if ($("#payment_bank_old").val() == content[i]) {
                    $("#payment_bank").append(
                        "<option value='" + content[i] + "' selected>" + data[i] +
                        "</option>"
                    );
                } else {
                    $("#payment_bank").append(
